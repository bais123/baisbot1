...
//━━━━━━━━━━━━━━━[ CONNECTION 1 ]━━━━━━━━━━━━━━━━━//
		...
		mess = {
			wait: 'Sabar Lagi Proses ya...',
			success: 'Sukses, tetap support bot ini',
			error: {
				stick: 'Gagal Convert Gambar To Sticker...Coba Lagi !',
				Iv: 'Linknya Error Kak...'
			},
			only: {
				admin: 'Khusus Admin Kak....',
...
switch (command) {
	...
	menu = `𝙃𝙖𝙮 ${pushname} ${tampilUcapan}

✉️ 𝙎𝙤𝙪𝙧𝙘𝙚 𝙘𝙤𝙙𝙚 = Chat owner
👤 𝘼𝙪𝙩𝙝𝙤𝙧 = Zero YT7
🌍 𝙇𝙞𝙗 = Baileys
...
costum(buffer, sticker, Verived, `TETAP SUPPORT BOT INI`)
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
...
//━━━━━━━━━━━━━━━[ FITUR MAKER ]━━━━━━━━━━━━━━━━━//
        ...
        case "googlem":
				...
				if (args.length < 1) return reply(`Teksnya Mana Tod ?\nContoh: ${prefix + command}Zero YT7 | ganteng | banget`)
				reply(mess.wait)
kontol = body.slice(8)
...
break
case 'tahta':
if (args.length < 1) return reply(`Teksnya Mana Kak ?\nContoh: ${prefix + command}Bais Bot`)
				dapuhy = body.slice(7)
				reply(mess.wait)
				...
				case "halloween":
					...
					break
					case "dance":
				if (args.length < 1) return reply(from, `Teksnya Mana Kak ?\nContoh: ${prefix + command}Bais Bot`, mek)
				...
				if (args.length < 1) return reply(from, `Teksnya Mana Kak ?\nContoh: ${prefix + command}BaisBot`, mek)
			 reply(mess.wait)
...
encmediam = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
		...
		damdu = fs.readFileSync(`./sticker/${random}.webp`)
			...
			case "caripesan":
        case "smsg":
        if (!q) return reply("Pesan apa kak ?");
...
//━━━━━━━━━━━━━━━[ FITUR RANDOM IMAGE ]━━━━━━━━━━━━━━━━━//
      ...
      case "aeshtetic":
      case "wallpaperanime":
      sendButton(from, `Random Image By Zero YT7`, `press the button below`, [
          {
...
break
case 'playmp3':
if (args.length == 0) return await reply(`Judul Lagunya Mana Kak ?\nContoh : ${prefix + command} melukis senja`)
reply(mess.wait)
